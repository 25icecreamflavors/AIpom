...
def main():
    ...
    if not skip_llm_training:
        ...
        print("--- Skipping LLM training as per config ---")
        if not os.path.exists(merged_labeled_train_path):
            raise FileNotFoundError(f"LLM training was skipped, but the labeled train file was not found at {merged_labeled_train_path}. Please run the full pipeline first or place the file there manually.")
