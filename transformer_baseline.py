...
class DatasetConfig:
    train_file: str = field(
        default=None, metadata={"help": "Path to train jsonl file"}
    )
    dev_file: str = field(
        default=None, metadata={"help": "Path to dev jsonl file"}
    )
    ...
    def __getitem__(self, idx):
        ...
        if len(input_ids) < self.max_length - 2:
            input_ids = (
                ...
                + input_ids
                + [2]
                + [1] * (self.max_length - len(input_ids) - 2)
            )
            if labels_available:
                labels = (
                    [-100]
                    + labels
                    + [-100] * (self.max_length - len(labels) - 1)
                )
...
def evaluate_machine_start_position(
    labels, predictions, idx2word=None, token_level=True
):
    """
...
def compute_metrics(p):
    pred, labels = p
    mean_absolute_diff = evaluate_machine_start_position(
        labels, pred, token_level=True
    )
...
if __name__ == "__main__":
    ...
    for layer in model.deberta.encoder.layer[:12]:
            ...
            df = pd.DataFrame(
                {
                    ...
                    "label": [
                        ...
                        for i in list(
                            zip(predictions.argmax(axis=-1), test_set)
                        )
                    ],
                }
            )
